    ...
    <div class="layui-body">
        ...
        <div class="form-panel">
            <div class="panel-heading">
                <h5 class="mb"><i class="fa fa-list-ul"></i>会所房间管理</h5>
            </div>
                        ...
                        <div class="content-panel">
                            <table id="table_data" class="dataTable table table-hover">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 20%;">房间类型</th>
                                    <th style="width: 20%;">楼层</th>
...
<script>
    ...
    layui.use(['element', 'layer', 'globalAjax', 'datatable', 'jquery', 'form'], function () {
            ...
            json.columns = [
                ...
                },
                {
                    ...
                    "mRender": function (data, type, row) {
                        ...
                        array.push("<button class=\"layui-btn layui-btn-primary layui-btn-sm toEdit\" vb=\"" + row.roomCode + "\">");
                        array.push("<i class=\"layui-icon\">&#xe642;</i>");
                        array.push("</button>")
                        </c:if>
                        <c:if test="${allow_delete==1}">
                        array.push("<button class=\"layui-btn layui-btn-primary layui-btn-sm deleteRoomCode\" vb=\"" + row.roomCode + "\">");
                        array.push("<i class=\"layui-icon\">&#xe640;</i>");
                        array.push("</button>")
                        </c:if>
                        array.push("</div>");
        ...
        $("#add").click(function () {
            window.location.href = "${ctx}/room/createRoom"
        })

        $(document).on('click', ".toEdit", function () {
            var roomCode = $(this).attr("vb");
            window.location.href = "${ctx}/room/" + roomCode + "/updateRoom"
        });
        $(document).on('click', ".deleteRoomCode", function () {
            var roomCode = $(this).attr("vb");
            layer.confirm('是否删除？', {icon: 3, title: '提示'}, function (index) {
                    $.ajax({
                        url: "${ctx}/room/" + roomCode + "/roomDelete",
                        type: "POST",
                        dataType : "json",
